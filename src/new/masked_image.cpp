  ...
  //TAKEN_TIME_US();
    ...
    for (int dy = -patch_size; dy <= patch_size; ++dy)
    {
        for (int dx = -patch_size; dx <= patch_size; ++dx)
        {
            int yy = y + dy;
            int xx = x + dx;
            if (yy >= 0 && yy < mask_size.height &&
                xx >= 0 && xx < mask_size.width)
            {
...
void MaskedImage::compute_image_gradients()
{
  //TAKEN_TIME_US();
    ...
    m_image_grady = cv::Mat::zeros(size, CV_8UC3);
    m_image_gradx = cv::Mat::zeros(size, CV_8UC3);
//    m_image_grady = cv::Scalar::all(0);
//    m_image_gradx = cv::Scalar::all(0);

    for (int i = 1; i < size.height - 1; ++i)
    {
        const auto *ptr = m_image.ptr<uint8_t>(i, 0);
        const auto *ptry1 = m_image.ptr<uint8_t>(i + 1, 0);
        const auto *ptry2 = m_image.ptr<uint8_t>(i - 1, 0);
        const auto *ptrx1 = m_image.ptr<uint8_t>(i, 0) + 3;
        const auto *ptrx2 = m_image.ptr<uint8_t>(i, 0) - 3;
        auto *mptry = m_image_grady.ptr<uint8_t>(i, 0);
        auto *mptrx = m_image_gradx.ptr<uint8_t>(i, 0);
