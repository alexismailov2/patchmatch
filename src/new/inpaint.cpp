...
Inpainting::Inpainting(cv::Mat image, cv::Mat mask, cv::Mat global_mask, const PatchDistanceMetric *metric)
    : m_initial(image, mask, global_mask)
    , m_distance_metric(metric)
    , m_pyramid()
    , m_source2target()
    , m_target2source()
...
cv::Mat Inpainting::run(bool verbose, pm::PatchMatch::Progress::Cb verbose_visualize, unsigned int random_seed)
{
    TAKEN_TIME_US();
    srand(random_seed);
    ...
    pm::PatchMatch::Progress progress{0, static_cast<uint8_t>(nr_levels)};
...
//            cv::imshow("Source", source_visualize);
            ...
            cv::Mat target_visualize(m_initial.size(), m_initial.image().type());
            cv::resize(target.image(), target_visualize, m_initial.size());
            progress.currentScale = nr_levels - level;
            if (!verbose_visualize(target_visualize, progress))
            {
              break;
            }
...
void Inpainting::_expectation_step(const NearestNeighborField &nnf, bool source2target, cv::Mat &vote, const MaskedImage &source, bool upscaled)
{
  TAKEN_TIME_US();
    ...
    for (auto i = 0; i < source_size.height; ++i)
    {
        for (auto j = 0; j < source_size.width; ++j)
        {
            ...
            for (int di = -patch_size; di <= patch_size; ++di)
            {
                for (int dj = -patch_size; dj <= patch_size; ++dj)
                {
                    ...
                    if (!(ys >= 0 && ys < source_size.height && xs >= 0 && xs < source_size.width) ||
                        !(yt >= 0 && yt < target_size.height && xt >= 0 && xt < target_size.width) ||
                        nnf.source().is_globally_masked(ys, xs) ||
                        nnf.target().is_globally_masked(yt, xt))
                    {
                      continue;
                    }
