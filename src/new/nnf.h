...
public:
  NearestNeighborField() = default;
    ...
    NearestNeighborField(const MaskedImage &source, const MaskedImage &target, const PatchDistanceMetric *metric, const NearestNeighborField &other, int max_retry = 20)
        ...
        , m_target(target)
        , m_distance_metric(metric)
        , m_field{cv::Mat::zeros(m_source.size(), CV_32SC3)}
    {
...
private:
    int _distance(int source_y, int source_x, int target_y, int target_x) {
...
class DebugPatchSSDDistanceMetric
  : public PatchDistanceMetric
{
public:
    DebugPatchSSDDistanceMetric(int patch_size, int width, int height)
        : PatchDistanceMetric(patch_size)
        , m_width(width)
        , m_height(height)
    {}
    virtual int operator ()(const MaskedImage &source, int source_y, int source_x, const MaskedImage &target, int target_y, int target_x) const;
...
class RegularityGuidedPatchDistanceMetricV1
  : public PatchDistanceMetric
{
public:
...
class RegularityGuidedPatchDistanceMetricV2
  : public PatchDistanceMetric
{
public:
